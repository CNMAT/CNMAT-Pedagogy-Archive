...
function set_thresh(t)
{
	thresh = t;
}

function set_nsamps(i)
{
	thresh = i;
}

function get_peaks(bufname)
{
	buf = new Buffer(bufname)
	
	var peaksamples = new Array();
	var peakcount = 0;
	...
	{
		var samps = buf.peek(1, i, nsamps);
		var sum = samps.reduce( function(a, b){ return a+b; } );
			
		rms = Math.sqrt( sum * sum );
		
		var delta = rms - prevrms;
